...
import kotlinx.android.synthetic.main.item_aksara_lontara.view.textview_lontara_bugis
import kotlinx.android.synthetic.main.item_aksara_lontara.view.textview_lontara_bugis_arti
import kotlinx.android.synthetic.main.item_diaktrik.view.*

/**
 * Created by Sainal Sultan on 6/01/2020.
 * sainalsultan@gmail.com | Hasanah Dev.
 */
class AdapterDiaktrik : RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private val list : MutableList<String> = mutableListOf()
    private val listLontara : MutableList<String> = mutableListOf()
    private val listImgLontara : MutableList<Int> = mutableListOf()

    fun setAdapter(
        list: MutableList<String>,
        listLontara: MutableList<String>,
        listImgLontara: MutableList<Int>
    ){
        this.list.clear()
        this.list.addAll(list)
        this.listLontara.clear()
        this.listLontara.addAll(listLontara)
        this.listImgLontara.clear()
        this.listImgLontara.addAll(listImgLontara)
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        LayoutInflater.from(parent.context).inflate(R.layout.item_diaktrik,parent,false).also {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ViewHolder){
            holder.itemView.run{
                ...
                imageview_diaktrik.setImageResource(listImgLontara.get(position))
            }
        }
    }
